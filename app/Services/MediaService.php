...
class MediaService extends AbstractService
{
    ...
    public function uploadMedia($request)
    {
        ...
        if (!empty($name)) {
            ...
            $media_path = $folder . '/' . $savedName;               // modified media_path to relative path
            ...
            if (!empty($saved) && is_object($saved)) {
                $data_insert = [
                    ...
                    // 'media_path' => $saved->getPathname(),       // absolute path
                    'media_path' => $media_path,                    // relative path
                    'media_size' => $size,
    ...
    public function getMediaFolder($storage = false)
    {
        ...
        $date = date('d');
        if ($storage) {
            // return asset('storage/' . $year . '/' . $month . '/' . $date);   // localhost:8000/storage/2024/07/22
            return '/storage/' . $year . '/' . $month . '/' . $date;            // /storage/2024/07/22
        } else {
